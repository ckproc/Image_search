...
public:
   ...
   void getAffineHessianDescriptor( Mat& descriptor, Mat& frame) 
   {
     int keyPointSize = keys.size();
     int siftDescriptorCount = 128;
     
     descriptor.release();
     frame.release();
     
     descriptor = Mat( keyPointSize, siftDescriptorCount, CV_32FC1, Scalar::all(0.0) );
     frame = Mat( 3, keyPointSize, CV_32FC1, Scalar::all(0.0) );
     for (size_t i=0; i<keyPointSize; i++)
     {
       Keypoint &k = keys[i];
       
       frame.ptr<float>(0)[i] = k.x;
       frame.ptr<float>(1)[i] = k.y;
       frame.ptr<float>(2)[i] = k.s;
       
       for (size_t j=0; j<siftDescriptorCount; j++)
       {
         descriptor.ptr<float>(i)[j] = int(k.desc[j]);
       }
     }
   }
};

void getAffineHessianDescriptor(Mat& tmp, Mat& descriptor, Mat& frame)
{
    ...
    double t1 = 0;
    {
         ...
         detector.getAffineHessianDescriptor(descriptor, frame);
    }
